...
class LangevinThermostat(ut.TestCase):
    ...
    def global_langevin_run_check(self, N, kT, loops):
        ...
        if espressomd.has_features("ROTATION") and "LANGEVIN_IMPULSE" not in espressomd.features():
            self.check_velocity_distribution(
    ...
    @ut.skipIf(not espressomd.has_features("ERMAK_BUCKHOLZ"), "Skipped for lack of ERMAK_BUCKHOLZ")
    def test_021__friction_trans(self):
        """Tests the translational friction-only part of the ERMAK-BUCKHOLZ thermostat."""
        ...
        system.thermostat.turn_off()
        if espressomd.has_features("PARTICLE_ANISOTROPY"):
            system.thermostat.set_eb(kT=0, gamma=gamma_t_a, seed=41)
        else:
            system.thermostat.set_eb(kT=0, gamma=gamma_t_i, seed=41)

        system.time = 0
        for i in range(100):
            system.integrator.run(10)
            #print("vx={0}".format(system.part[0].v[0]))
            if espressomd.has_features("PARTICLE_ANISOTROPY"):
                    np.testing.assert_allclose(
                        np.copy(system.part[0].v), v0 * np.exp(-gamma_t_a / system.part[0].mass * system.time), rtol=5E-2)
            else:
                    np.testing.assert_allclose(
                        np.copy(system.part[0].v), v0 * np.exp(-gamma_t_i / system.part[0].mass * system.time), atol=45E-4)

    @ut.skipIf(not espressomd.has_features("ERMAK_BUCKHOLZ"), "Skipped for lack of ERMAK_BUCKHOLZ")
    def test_022__friction_trans(self):
        """Tests the translational friction-only part of the ERMAK-BUCKHOLZ (option 2) thermostat."""
        ...
        system.thermostat.turn_off()
        if espressomd.has_features("PARTICLE_ANISOTROPY"):
            system.thermostat.set_eb_opt2(kT=0, gamma=gamma_t_a, seed=41)
        else:
            system.thermostat.set_eb_opt2(kT=0, gamma=gamma_t_i, seed=41)
    ...
    @ut.skipIf(not espressomd.has_features("LANGEVIN_IMPULSE"), "Skipped for lack of LANGEVIN_IMPULSE")
    def test_023__friction_trans(self):
        """Tests the translational friction-only part of the LANGEVIN_IMPULSE thermostat."""
        ...
        system.thermostat.turn_off()
        if espressomd.has_features("PARTICLE_ANISOTROPY"):
            system.thermostat.set_langevin_impulse(kT=0, gamma=gamma_t_a, seed=41)
        else:
            system.thermostat.set_langevin_impulse(kT=0, gamma=gamma_t_i, seed=41)
    ...
    @ut.skipIf(not espressomd.has_features("LANGEVIN_IMPULSE"), "Skipped for lack of LANGEVIN_IMPULSE")
    def test_024__friction_trans(self):
        """Tests the translational friction-only part of the vGB82 thermostat."""
        ...
        system.thermostat.turn_off()
        if espressomd.has_features("PARTICLE_ANISOTROPY"):
            system.thermostat.set_vanGunsteren_Berendsen(kT=0, gamma=gamma_t_a, seed=41)
        else:
            system.thermostat.set_vanGunsteren_Berendsen(kT=0, gamma=gamma_t_i, seed=41)
    ...
    @ut.skipIf(not espressomd.has_features(["ROTATION",
                                            "ERMAK_BUCKHOLZ"]),
               "Skipped for lack of ROTATION and/or ERMAK_BUCKHOLZ")
    def test_031__friction_rot(self):
        """Tests the rotational friction-only part of the ERMAK-BUCKHOLZ thermostat."""
        ...
        for i in range(100):
            ...
            if espressomd.has_features("PARTICLE_ANISOTROPY"):
                    np.testing.assert_allclose(
                        np.copy(system.part[0].omega_body), o0 * np.exp(-gamma_r_a / rinertia * system.time), rtol=5E-2)
            else:
                    np.testing.assert_allclose(
                        np.copy(system.part[0].omega_body), o0 * np.exp(-gamma_r_i / rinertia * system.time), atol=5E-4)
    ...
    @ut.skipIf(not espressomd.has_features(["ROTATION",
                                            "ERMAK_BUCKHOLZ"]),
               "Skipped for lack of ROTATION and/or ERMAK_BUCKHOLZ")
    def test_032__friction_rot(self):
        """Tests the rotational friction-only part of the ERMAK-BUCKHOLZ (option 2) thermostat."""
    ...
    def test_04__global_langevin(self):
        ...
        if espressomd.has_features("ERMAK_BUCKHOLZ"):
            # Large time-step is OK for EB.
            system.time_step = 7.214
            system.part[:].v = np.zeros((3))
            system.part[:].omega_body = np.zeros((3))
            system.thermostat.turn_off()
            system.thermostat.set_eb(kT=kT, gamma=gamma, seed=41)
            # Warmup
            # The EB does not require so the warmup. Only 1 step is enough.
            # More steps are taken just to be sure that they will not lead
            # to wrong results.
            system.integrator.run(3)
            # Less number of loops are needed in case of EB because the velocity
            # distribution is already as required. It is not a result of a real
            # dynamics.
            self.global_langevin_run_check(N, kT, 70)
            system.thermostat.turn_off()
        ...
        if espressomd.has_features("ERMAK_BUCKHOLZ"):
            ...
            system.part[:].omega_body = np.zeros((3))
            system.thermostat.turn_off()
            system.thermostat.set_eb_opt2(kT=kT, gamma=gamma, seed=41)
            # Warmup
            # The EB does not require so the warmup. Only 1 step is enough.
            # More steps are taken just to be sure that they will not lead
            # to wrong results.
        ...
        if espressomd.has_features("LANGEVIN_IMPULSE"):
            # Large time-step is OK for LI.
            system.time_step = 7.214
            system.part[:].v = np.zeros((3))
            system.part[:].omega_body = np.zeros((3))
            system.thermostat.turn_off()
            system.thermostat.set_langevin_impulse(kT=kT, gamma=gamma, seed=41)
            # Warmup
            # The EB does not require so the warmup. Only 1 step is enough.
            # More steps are taken just to be sure that they will not lead
            # to wrong results.
        ...
        if espressomd.has_features("LANGEVIN_IMPULSE"):
            # Small time-step is OK for LI also.
            system.time_step = 0.06
            system.part[:].v = np.zeros((3))
            system.part[:].omega_body = np.zeros((3))
            system.thermostat.turn_off()
        ...
        if espressomd.has_features("LANGEVIN_IMPULSE"):
            ...
            system.part[:].omega_body = np.zeros((3))
            system.thermostat.turn_off()
            system.thermostat.set_vanGunsteren_Berendsen(kT=kT, gamma=gamma, seed=41)
            # Warmup
            # The EB does not require so the warmup. Only 1 step is enough.
            # More steps are taken just to be sure that they will not lead
            # to wrong results.
        ...
        if espressomd.has_features("LANGEVIN_IMPULSE"):
            ...
            # distribution is already as required. It is not a result of a real
            # dynamics.
            self.global_langevin_run_check(N, kT, 300)
            system.thermostat.turn_off()
    ...
    @ut.skipIf(not espressomd.has_features("ERMAK_BUCKHOLZ"), "Skipped for lack of ERMAK_BUCKHOLZ")
    def test_061__diffusion(self):
        """This tests rotational and translational diffusion coeff via green-kubo
        for the ERMAK-BUCKHOLZ thermostat"""
        system = self.system
        system.part.clear()
    ...
    @ut.skipIf(not espressomd.has_features("ERMAK_BUCKHOLZ"), "Skipped for lack of ERMAK_BUCKHOLZ")
    def test_062__diffusion(self):
        """This tests rotational and translational diffusion coeff via green-kubo
        for the ERMAK-BUCKHOLZ (option 2) thermostat"""
        system = self.system
        system.part.clear()
    ...
    @ut.skipIf(not espressomd.has_features("LANGEVIN_IMPULSE"), "Skipped for lack of LANGEVIN_IMPULSE")
    def test_064__diffusion(self):
        """This tests rotational and translational diffusion coeff via green-kubo
        for the LANGEVIN_IMPULSE thermostat"""
        system = self.system
        system.part.clear()
    ...
    @ut.skipIf(not espressomd.has_features("LANGEVIN_IMPULSE"), "Skipped for lack of LANGEVIN_IMPULSE")
    def test_065__diffusion(self):
        """This tests rotational and translational diffusion coeff via green-kubo
        for the vGB82 thermostat"""
        system = self.system
        system.part.clear()
